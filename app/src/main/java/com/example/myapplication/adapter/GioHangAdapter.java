...
import com.example.myapplication.Interface.IImageClickListenner;
...
public class GioHangAdapter extends RecyclerView.Adapter<GioHangAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.setListenner(new IImageClickListenner() {
            //Nhấn lúc tăng giảm số lượng
            @Override
            public void onImageClick(View view, int pos, int giaitri) {
                Log.d("TAG", "onImangeClick: "+pos+" ..."+giaitri);
                if(giaitri == 1){
                    ...
                    //Xóa sản phẩm trong giỏ hàng và phải cập nhật cả bên chi tiết nhé
                    else if(gioHangList.get(pos).getSoluong() == 1){
                        AlertDialog.Builder builder = new AlertDialog.Builder(view.getRootView().getContext());
                        builder.setTitle("Thông báo");
                        builder.setMessage("Bạn có muốn xóa sản phẩm này trong giỏ hàng");
                        builder.setPositiveButton("Đồng ý", new DialogInterface.OnClickListener(){

                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Utils.manggiohang.remove(pos);
                                notifyDataSetChanged(); // xóa đi r kh cần phải set lại gia
                                EventBus.getDefault().postSticky(new TinhTongEvent());
                            }
                        });
